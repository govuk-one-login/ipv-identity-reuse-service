Feature: UserIdentity Driving Licence Expiry Check
  ...
  Scenario: Record is not expired when failed DCMAW driving permit VC has expired licence older than 180 days
    Given the user has a failed DCMAW driving permit credential with licence expired before VC issuance and VC nbf 7 months ago
    And the user has a fraud check credential with nbf 3 months ago
    And the user has a stored identity, with VOT "P2"
    When I make a request for the users identity with a VTR "P2"
    Then the status code should be 200
  ...
  Scenario: Record is not expired when driving licence has future expiry but VC is older than 180 days
    Given the user has a DCMAW driving permit credential with licence expiry in the future and VC nbf 7 months ago
    And the user has a fraud check credential with nbf 3 months ago
    And the user has a stored identity, with VOT "P2"
    When I make a request for the users identity with a VTR "P2"
    Then the status code should be 200
  ...
  Scenario: Record is not expired when bundle has failed DCMAW driving permit and successful DCMAW passport
    Given the user has a failed DCMAW driving permit credential with licence expired before VC issuance and VC nbf 7 months ago
    And the user has a DCMAW passport credential with VC nbf 3 months ago
    And the user has a fraud check credential with nbf 3 months ago
    And the user has a stored identity, with VOT "P2"
    When I make a request for the users identity with a VTR "P2"
    Then the status code should be 200
