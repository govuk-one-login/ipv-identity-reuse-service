...
describe("api-gateway-bearer-token-authorizer-handler", () => {
  ...
  describe("return a allow policy", () => {
    it("should return an allow policy and subject and storageToken when bearer token is present and valid", async () => {
      ...
      expect(response.principalId).toBe(sha256Hash(subjectId));
    ...
    it("should return an allow policy and only subject when bearer token is present and valid", async () => {
      const accessToken = "abcd1234";
      const subjectId = "urn:uuid:569faa50-71fe-4d12-b54f-9809483fec0c";
      const dynamodbDocumentMock = mockClient(DynamoDBDocumentClient);
      dynamodbDocumentMock.on(QueryCommand).resolves({
  ...
  describe("return an unauthorized error", () => {
    it("should return an unauthorized error when no authorisation header present", async () => {
      const event = createMockEvent({});
    ...
    it("should return an unauthorized error when authorisation header is not a bearer token", async () => {
      const event = createMockEvent({
        headers: {
    ...
    it("should return an unauthorized error when access token not found", async () => {
      const dynamodbDocumentMock = mockClient(DynamoDBDocumentClient);
      dynamodbDocumentMock.on(QueryCommand).resolves({
        Count: 0,
        Items: [],
    ...
    it("should return an unauthorized error when multiple access token matches found", async () => {
      const dynamodbDocumentMock = mockClient(DynamoDBDocumentClient);
      dynamodbDocumentMock.on(QueryCommand).resolves({
        Count: 1,
  ...
  describe("unexpected exceptions", async () => {
    it("should throw if Dynamo throws exception", async () => {
      ...
      await expect(handler(event, {} as Context)).rejects.toThrow("Unexpected error");
