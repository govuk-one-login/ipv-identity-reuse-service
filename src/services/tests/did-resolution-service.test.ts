...
import {
  ...
  isValidDidWeb,
  resolver,
} from "../did-resolution-service-api.js";
...
describe("isValidDidWeb", () => {
  ...
  ])("should match valid did:web IDs", (kid: string) => {
  ...
  ])("should not match invalid did:web IDs", (kid: string) => {
    expect(isValidDidWeb(kid)).toEqual(false);
  });
});

describe("getDidWebController", () => {
  it("should correctly extract the controller ID", () => {
    expect(getDidWebController("did:web:example.com#key-1")).toEqual("example.com");
  });
  it("should correctly extract the controller ID including path portion", () => {
